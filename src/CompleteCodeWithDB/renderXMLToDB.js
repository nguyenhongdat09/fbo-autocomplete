...
class RenderXMLToDB extends ana {
    ...
    getPathDirGridFiler() {
        ...
        var controllersSep = path.sep + 'Controllers' + path.sep;
        var idx = pathFile.indexOf(controllersSep);

        // Fallback: try the other separator
        if (idx === -1) {
            controllersSep = (path.sep === '\\') ? '/Controllers/' : '\\Controllers\\';
            idx = pathFile.indexOf(controllersSep);
        }

        if (idx === -1)
            return []

        var pathToControllers = pathFile.substring(0, idx + controllersSep.length);
