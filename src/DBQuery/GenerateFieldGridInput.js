...
const { buildGridInputFallbackFieldXml } = require("./fieldXmlSqlFallback");

const LEVEL_FOLDER = "GridInput";
 ...
 */
async function buildHeaderToGridInputXml(orderedColumns, fieldVariant) {
    const variant =
    ...
    for (let i = 0; i < list.length; i++) {
        ...
        else parts.push(buildGridInputFallbackFieldXml(list[i]));
    }
    return parts.join("\n");
}

module.exports = {
    buildHeaderToGridInputXml,
};
