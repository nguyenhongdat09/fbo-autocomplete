...
<!DOCTYPE grid [
  ...
  select top 0 @@fieldExternal from {{src_detail_table}} a, {{src_master_table}} m, dmvt b
  return">
]>
...
select @queryFormClause = '{{src_master_table}} m with(nolock) join {{src_detail_table}} a with(nolock) on m.stt_rec = a.stt_rec left join dmvt b with(nolock) on a.ma_vt = b.ma_vt'
