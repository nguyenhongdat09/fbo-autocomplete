...
<dir table="{{src_master_table}}" code="stt_rec" order="ngay_ct, so_ct" id="{{src_ma_ct}}" xmlns="urn:schemas-fast-com:data-dir">
...
if @stt_rec_ct <> '' begin
  ...
  select top 0 stt_rec into #t from {{src_inquiry_table}}
  set @s = 'insert into #t select top 1 stt_rec from {{src_inquiry_table}}'
...
Retrieve:
select @vcID = @retrieveID, @d = ngay_ct from {{src_master_table}} where stt_rec = @retrieveID
select cast('' as nvarchar(512)) as fields into #r from {{src_master_table}}
select @s = 'insert into #r select ' + @fields + ' as fields from {{src_master_table}} where stt_rec = ''' + replace(@retrieveID, '''', '') + ''''
exec sp_executesql @s
